...
def simulate_with_processing_delay():
    ...
    for batch in tardis.stream_book(product, times, depth=1, batch_size=1):
        ...
        if batch.height > 0:
            # Extract data
            mid_price = ((batch.select(pl.col("bids[0].price")) + 
                        batch.select(pl.col("asks[0].price"))) / 2).item()
            ...
            if tick_count % 10 == 0:
                ...
                print(f"Tick {tick_count}: Price={mid_price:.2f} "
...
def simulate_replay_at_original_speed():
    ...
    for batch in tardis.stream_book(product, times, depth=1, batch_size=1):
        ...
        if batch.height > 0:
            ...
            ts_readable = pl.from_epoch([ts_ms], time_unit="ms").dt.strftime("%H:%M:%S.%3f").item()
    ...
    for batch in tardis.stream_book(product, times, depth=1, batch_size=1):
        ...
        if current_time - last_sample_time >= sample_interval:
            ...
            if current_batch is not None and current_batch.height > 0:
                mid_price = ((current_batch.select(pl.col("bids[0].price")) + 
                            current_batch.select(pl.col("asks[0].price"))) / 2).item()
        ...
        if len(batch_buffer) >= batch_size:
            ...
            for i, tick_batch in enumerate(batch_buffer):
                if tick_batch.height > 0:
                    mid_price = ((tick_batch.select(pl.col("bids[0].price")) + 
                                tick_batch.select(pl.col("asks[0].price"))) / 2).item()
                    print(f"  Tick {tick_count - batch_size + i + 1}: Price={mid_price:.2f}")
