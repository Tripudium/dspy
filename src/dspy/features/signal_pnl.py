...
def add_sig_pnl(
    ...
    df_shift = (
        df.select(
            [
                (pl.col(ts_col) + tdelta).alias("ts").set_sorted(),
                pl.col(col),
                pl.col(signal),
            ]
        )
        .join_asof(fut_df, on=ts_col, strategy="backward")
        .select((expr_diff).alias(f"pnl_sig_{horizon}"))
    )
...
def create_signal_test_dataframes():
    ...
    df = pl.DataFrame({"ts": base_ts, "prc": np.random.normal(100, 5, 20)})
    ...
    signal_df = pl.DataFrame({"ts": signal_ts, "signal": [1, -1, 2, -2, 3, -3]})
