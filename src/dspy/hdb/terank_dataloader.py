"""
Module for loading and processing Tardis data obtained via Terank.
This module provides functionality to access and manipulate Tardis datasets.
"""

import logging
from datetime import datetime

import polars as pl
from polars import col as c

from trpy_data.features import add_mid, add_vwap
from trpy_data.hdb.utils import round_up_to_nearest, str_to_timedelta
from trpy_data.utils.polars.lazy_frame import TrnkLazyFrame

from .registry import register_dataset

logger = logging.getLogger(__name__)


@register_dataset("terank")
class TerankData:
    """
    Dataloader for Terank data (obtained via Terank)
    """

    def __init__(self):
        logger.info("Initializing TerankDataLoader")

    def load_book(
        self, products: list[str], times: list[str], depth: int = 1, lazy=False
    ) -> pl.DataFrame:
        """
        Load book data for a given product and times.
        """
        if isinstance(products, str):
            products = [products]
        timestr = f"{times[0][:-2]}:{times[1][:-2]}"

        dfs = []

        for product in products:
            prodname = product + "__BNCE_USDTM"
            if depth == 1:
                df = TrnkLazyFrame.load_contract(
                    update_type="bbo", contract_spec=prodname, ts_range=timestr
                )
                df = df.select(
                    [
                        c("tsr").alias("ts"),
                        c("prc_s0"),
                        c("prc_s1"),
                        c("vol_s0"),
                        c("vol_s1"),
                    ]
                )
            else:
                df = TrnkLazyFrame.load_contract(
                    update_type="depth", contract_spec=prodname, ts_range=timestr
                )
                df = df.select(
                    [
                        c("tsr").alias("ts"),
                        c("prc_s0"),
                        c("prc_s1"),
                        c("vols_s0"),
                        c("vols_s1"),
                    ]
                )
                # TODO: Preprocess for different depths
            if lazy:
                columns = df.collect_schema()
            else:
                df = df.collect()
                columns = df.columns
            rename_map = {col: f"{col}_{product}" for col in columns if col != "ts"}
            df = df.rename(rename_map)
            dfs.append(df)
        merged_df = (
            pl.concat([df.select("ts") for df in dfs], how="vertical").unique("ts").sort("ts")
        )
        for i, df in enumerate(dfs):
            merged_df = merged_df.join_asof(df, on="ts")
        return merged_df.drop_nulls().sort("ts")

    def load_trades(self, products: list[str] | str, times: list[str], lazy=False) -> pl.DataFrame:
        """
        Load trades data for a given product and times.
        """
        if isinstance(products, str):
            products = [products]
        timestr = f"{times[0][:-2]}:{times[1][:-2]}"

        dfs = []
        for product in products:
            prodname = product + "__BNCE_USDTM"
            df = TrnkLazyFrame.load_contract(
                update_type="trade", contract_spec=prodname, ts_range=timestr
            )
            df = df.select(
                [
                    c("tsr").alias("ts"),
                    c("prc"),
                    c("qty"),
                    c("blended_id").alias("trade_id"),
                ]
            )
            df = df.with_columns(pl.lit(product).alias("product"))
            dfs.append(df)
        tdf = pl.concat(dfs).sort("ts")
        if not lazy:
            tdf = tdf.collect()
        return tdf

    def load(
        self, products: list[str], times: list[str], col: str = "mid", freq: str = "1s", lazy=False
    ) -> pl.DataFrame:
        """
        Load data for a given product and times.
        """
        df = self.load_book(products, times, lazy=lazy)
        df = df.with_columns([pl.from_epoch(c("ts"), time_unit="ns").alias("dts")])
        dtimes = [datetime.strptime(t, "%y%m%d.%H%M%S") for t in times]
        try:
            td = str_to_timedelta(freq)
        except ValueError:
            raise ValueError(f"Invalid frequency: {freq}")

        min_dt = round_up_to_nearest(df["dts"][0], td)
        max_dt = dtimes[1]

        # Make sure that every timestamp is present in the dataframe
        rdf = pl.DataFrame(
            {"dts": pl.datetime_range(min_dt, max_dt, freq, time_unit="ns", eager=True)}
        )
        rdf = rdf.join_asof(df, on="dts", strategy="backward")

        if col == "mid":
            rdf = add_mid(rdf, cols=["prc_s0", "prc_s1"])
            if len(products) > 1:
                rdf = rdf.select(
                    [pl.col("dts").alias("ts")] + [pl.col(f"mid_{product}") for product in products]
                )
            else:
                rdf = rdf.select([pl.col("dts").alias("ts"), pl.col("mid")])
        elif col == "vwap":
            rdf = add_vwap(rdf, cols=["prc_s0", "prc_s1", "vol_s0", "vol_s1"])
            if len(products) > 1:
                rdf = rdf.select(
                    [pl.col("dts").alias("ts")]
                    + [pl.col(f"vwap_{product}") for product in products]
                )
            else:
                rdf = rdf.select([pl.col("dts").alias("ts"), pl.col("vwap")])
        return rdf
